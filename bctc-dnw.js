...
async function fetchAndExtractData() {
  try {
    const response = await axios.get(
      'https://gateway.fpts.com.vn/news/api/gateway/v1/mobile/list?folder=86&code=DNW&pageSize=8&selectedPage=1&cbtt=1&from=01-01-1970&to=01-01-3000&newsType=1',
      {
        headers: {
    ...
    if (newNames.length) {
      ...
      await Promise.all(
        newNames.map(name =>
          sendTelegramNotification(`Báo cáo tài chính của DNW::: ${name}`)
        )
      );
