...
async function fetchAndExtractData() {
  try {
    const response = await axios.get('https://domesco.com/quan-he-nha-dau-tu/quan-he-co-dong.html', {
      headers: {
        'accept': 'text/html',
    ...
    const newNames = await filterNewNames(names, COMPANIES.DMC);
    console.log('📢 [bctc-cdn.js:46]', newNames);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.DMC);

      // Gửi thông báo Telegram cho từng báo cáo mới
      await Promise.all(
        newNames.map(name => {
          return sendTelegramNotification(`Báo cáo tài chính của DMC ::: ${name}`);
        })
      );
