...
async function fetchAndExtractData() {
  try {
    const response = await axios.get('https://cholimexfood.com.vn/tin-co-dong/', {
      headers: {
        'accept': 'text/html',
    ...
    const names = [];
    $('.post-title.is-large a').each((_, el) => {
    ...
    const newNames = await filterNewNames(names, COMPANIES.CMF);
    console.log('📢 [bctc-cdn.js:46]', newNames);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.CMF);

      // Gửi thông báo Telegram cho từng báo cáo mới
      await Promise.all(
        newNames.map(name => {
          return sendTelegramNotification(`Báo cáo tài chính của CMF ::: ${name}`);
        })
      );
