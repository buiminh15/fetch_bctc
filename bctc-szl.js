...
async function fetchAndExtractData() {
  try {
    const response = await axios.get(`https://szl.com.vn/co-dong/`, {
      headers: {
        'accept': 'text/html',
    ...
    const names = [];
    $('#tab-content-25 span').each((index, element) => {
      const nameRaw = $(element).text().trim();
      const name = he.decode(nameRaw);
      if (index < 10) {
        const filterCondition = [currentYear, 'báo cáo'];
    ...
    const newNames = await filterNewNames(names, COMPANIES.SZL);
    console.log('📢 [bctc-cdn.js:46]', newNames);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.SZL);

      // Gửi thông báo Telegram cho từng báo cáo mới
      await Promise.all(
        newNames.map(name => {
          return sendTelegramNotification(`Báo cáo tài chính của SZL ::: ${name}`);
        })
      );
