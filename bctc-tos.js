...
async function fetchAndExtractData() {
  try {
    const response = await axios.get('https://tancangoffshore.com/invester_cat/bao-cao-tai-chinh/', {
      headers: {
        'accept': 'text/html',
    ...
    const names = [];
    $('.bh-article-list-item__title h2').each((_, el) => {
    ...
    const newNames = await filterNewNames(names, COMPANIES.TOS);
    console.log('📢 [bctc-cdn.js:46]', newNames);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.TOS);

      // Gửi thông báo Telegram cho từng báo cáo mới
      await Promise.all(
        newNames.map(name => {
          return sendTelegramNotification(`Báo cáo tài chính của TOS ::: ${name}`);
        })
      );
