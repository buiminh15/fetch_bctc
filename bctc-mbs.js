...
async function fetchAndExtractData() {
  try {
    const response = await axios.get('https://mbs.com.vn/bao-cao-tai-chinh/', {
      headers: {
        'accept': 'text/html',
    ...
    const names = [];
    $('h3.lg\\:text-xl.text-sm.text-body.font-medium a').each((_, element) => {
    ...
    const newNames = await filterNewNames(names, COMPANIES.MBS);
    console.log('📢 [bctc-cdn.js:46]', newNames);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.MBS);

      // Gửi thông báo Telegram cho từng báo cáo mới
      await Promise.all(
        newNames.map(name => {
          return sendTelegramNotification(`Báo cáo tài chính của MBS ::: ${name}`);
        })
      );
