...
async function fetchAndExtractData() {
  try {
    ...
    const response = await axios.get(`https://hoatho.com.vn/quan-he-co-dong/tinh-hinh-tai-chinh/${currentYear}-1`, {
      headers: {
        'accept': 'text/html',
    ...
    $('.content').each((_, el) => {
      ...
      const nameRaw = $(firstA).text().trim();
    ...
    const newNames = await filterNewNames(names, COMPANIES.HTG);
    console.log('📢 [bctc-cdn.js:46]', newNames);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.HTG);

      // Gửi thông báo Telegram cho từng báo cáo mới
      await Promise.all(
        newNames.map(name => {
          return sendTelegramNotification(`Báo cáo tài chính của HTG ::: ${name}`);
        })
      );
