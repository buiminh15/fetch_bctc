...
async function fetchAndExtractData() {
  try {
    ...
    const newNames = await filterNewNames(names, COMPANIES.BCF);
    console.log('📢 [bctc-cdn.js:46]', newNames);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.BCF);

      // Gửi thông báo Telegram cho từng báo cáo mới
      await Promise.all(
        newNames.map(name => {
          return sendTelegramNotification(`Báo cáo tài chính của BCF ::: ${name}`);
        })
      );
