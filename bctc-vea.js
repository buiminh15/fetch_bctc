...
async function fetchAndExtractData() {
  try {
    const response = await axios.get('http://veamcorp.com/quan-he-co-dong/bao-cao-tai-chinh-113.html', {
      headers: {
        'accept': 'text/html',
    ...
    const newNames = await filterNewNames(names, COMPANIES.VEA);
    console.log('📢 [bctc-cdn.js:46]', newNames);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.VEA);

      // Gửi thông báo Telegram cho từng báo cáo mới
      await Promise.all(
        newNames.map(name => {
          return sendTelegramNotification(`Báo cáo tài chính của VEA ::: ${name}`);
        })
      );
