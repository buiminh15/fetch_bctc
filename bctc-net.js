...
async function fetchAndExtractData() {
  try {
    const response = await axios.get('https://www.netcovn.com.vn/bao-cao-tai-chinh', {
      headers: {
        'accept': 'text/html',
    ...
    const names = [];
    $('tr').each((_, tr) => {
      // Lấy thẻ <a> đầu tiên trong <td> đầu tiên của mỗi <tr>
    ...
    const newNames = await filterNewNames(names, COMPANIES.NET);
    console.log('📢 [bctc-cdn.js:46]', newNames);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.NET);

      // Gửi thông báo Telegram cho từng báo cáo mới
      await Promise.all(
        newNames.map(name => {
          return sendTelegramNotification(`Báo cáo tài chính của NET ::: ${name}`);
        })
      );
