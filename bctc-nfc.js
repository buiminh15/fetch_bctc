...
async function fetchAndExtractData() {
  try {
    const response = await axios.get(`http://niferco.com.vn/quan-he-co-dong-tt3944.html?nn=vn`, {
      headers: {
        'accept': 'text/html',
    ...
    const newNames = await filterNewNames(names, COMPANIES.NFC);
    console.log('📢 [bctc-cdn.js:46]', newNames);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.NFC);

      // Gửi thông báo Telegram cho từng báo cáo mới
      await Promise.all(
        newNames.map(name => {
          return sendTelegramNotification(`Báo cáo tài chính của NFC ::: ${name}`);
        })
      );
