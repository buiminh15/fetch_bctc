...
async function fetchAndExtractData() {
  try {
    const response = await axios.get('https://masanconsumer.com/quan-he-co-dong/thong-tin-tai-chinh/', {
      headers: {
        'accept': 'text/html',
    ...
    const newNames = await filterNewNames(names, COMPANIES.MCH);
    console.log('📢 [bctc-cdn.js:46]', newNames);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.MCH);

      // Gửi thông báo Telegram cho từng báo cáo mới
      await Promise.all(
        newNames.map(name => {
          return sendTelegramNotification(`Báo cáo tài chính của MCH ::: ${name}`);
        })
      );
