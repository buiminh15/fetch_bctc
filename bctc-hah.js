...
async function fetchAndExtractData() {
  try {
    const response = await axios.get('https://haiants.vn/bao-cao-tai-chinh-q8.html', {
      headers: {
        'accept': 'text/html',
    ...
    const newNames = await filterNewNames(names, COMPANIES.HAH);
    console.log('📢 [bctc-cdn.js:46]', newNames);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.HAH);

      // Gửi thông báo Telegram cho từng báo cáo mới
      await Promise.all(
        newNames.map(name => {
          return sendTelegramNotification(`Báo cáo tài chính của HAH ::: ${name}`);
        })
      );
