...
async function fetchAndExtractData() {
  try {
    ...
    $('a').each((i, el) => {
      ...
      if (text.startsWith('Báo cáo tài chính')) {
        names.push(text);
      }
    });
    ...
    console.log('📢 [bctc-hpg.js:35]', newNames);
    if (newNames.length) {
      ...
      await Promise.all(
        newNames.map(name => {
          return sendTelegramNotification(`Báo cáo tài chính của Hòa Phát::: ${name}`);
        })
      );
...
console.log('📢 [bctc-hpg.js:53]', 'running');
fetchAndExtractData();

async function fetchBCSXKD() {
  try {
    const response = await axios.get('https://www.hoaphat.com.vn/quan-he-co-dong/cong-bo-thong-tin?page=1', {
      headers: {
        'accept': 'text/html',
        'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/138.0.0.0 Safari/537.36',
      },
