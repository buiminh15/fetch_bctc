...
async function fetchAndExtractData() {
  try {
    const response = await axios.get('https://tancanglogistics.com/quan-he-co-dong/Pages/bao-cao-tai-chinh-280120160747.aspx', {
      headers: {
        'accept': 'text/html',
    ...
    const newNames = await filterNewNames(names, COMPANIES.TCL);
    console.log('📢 [bctc-cdn.js:46]', newNames);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.TCL);

      // Gửi thông báo Telegram cho từng báo cáo mới
      await Promise.all(
        newNames.map(name => {
          return sendTelegramNotification(`Báo cáo tài chính của TCL ::: ${name}`);
        })
      );
